 ...
 * pqiv
 *
 * Copyright (c) 2013-2018, Phillip Berndt
 *
 ...
 *
 *
 * GraphicsMagick wand backend
 * Cloned from ImageMagick backend
 *
 */
...
static gboolean file_type_gmwand_has_extension(file_t *file, const char *extension) {
...
cairo_status_t file_type_gmwand_read_data(void *closure, unsigned char *data, unsigned int length) {/*{{{*/
	unsigned char **pos = closure;
	memcpy(data, *pos, length);
	*pos += length;
	return CAIRO_STATUS_SUCCESS;
}/*}}}*/
void file_type_gmwand_update_image_surface(file_t *file) {/*{{{*/
	...
	size_t image_size;
	unsigned char *image_data = MagickWriteImageBlob(private->wand, &image_size);
	unsigned char *image_data_loc = image_data;

	private->rendered_image_surface = cairo_image_surface_create_from_png_stream(file_type_gmwand_read_data, &image_data_loc);

	MagickRelinquishMemory(image_data);
}/*}}}*/

BOSNode *file_type_gmwand_alloc(load_images_state_t state, file_t *file) {/*{{{*/
	G_LOCK(magick_wand_global_lock);

	if(file_type_gmwand_has_extension(file, ".pdf") || file_type_gmwand_has_extension(file, ".ps")) {
		...
		size_t image_size;
		const unsigned char *image_data = (const unsigned char *)g_bytes_get_data(image_bytes, &image_size);
...
void file_type_gmwand_free(file_t *file) {/*{{{*/
	g_slice_free(file_private_data_wand_t, file->private);
}/*}}}*/
void file_type_gmwand_load(file_t *file, GInputStream *data, GError **error_pointer) {/*{{{*/
	G_LOCK(magick_wand_global_lock);
	file_private_data_wand_t *private = file->private;
	...
	if(private->page_number > 0) {
		...
		MagickSetImageIndex(private->wand, private->page_number - 1);
	}
	else {
		...
		if(delay) {
			...
			if(!file_type_gmwand_has_extension(file, ".psd")) {
				//MagickWand *wand = MagickMergeImageLayers(private->wand, FlattenLayer);
				MagickWand *wand = MagickFlattenImages(private->wand);
				DestroyMagickWand(private->wand);
				private->wand = wand;
				MagickResetIterator(private->wand);
			}
	...
	file_type_gmwand_update_image_surface(file);
...
double file_type_gmwand_animation_initialize(file_t *file) {/*{{{*/
	file_private_data_wand_t *private = file->private;
	// The unit of MagickGetImageDelay is "ticks-per-second"
	return 1000. / MagickGetImageDelay(private->wand);
}/*}}}*/
double file_type_gmwand_animation_next_frame(file_t *file) {/*{{{*/
...
void file_type_gmwand_unload(file_t *file) {/*{{{*/
	G_LOCK(magick_wand_global_lock);
	file_private_data_wand_t *private = file->private;
...
void file_type_gmwand_draw(file_t *file, cairo_t *cr) {/*{{{*/
	file_private_data_wand_t *private = file->private;
...
static void file_type_gmwand_exit_handler() {/*{{{*/
	G_LOCK(magick_wand_global_lock);
	DestroyMagick();
	G_UNLOCK(magick_wand_global_lock);
}/*}}}*/

void file_type_gmwand_initializer(file_type_handler_t *info) {/*{{{*/
	...
	atexit(file_type_gmwand_exit_handler);
	...
	// Assign the handlers
	info->alloc_fn                 =  file_type_gmwand_alloc;
	info->free_fn                  =  file_type_gmwand_free;
	info->load_fn                  =  file_type_gmwand_load;
	info->unload_fn                =  file_type_gmwand_unload;
	info->draw_fn                  =  file_type_gmwand_draw;
	info->animation_initialize_fn  =  file_type_gmwand_animation_initialize;
	info->animation_next_frame_fn  =  file_type_gmwand_animation_next_frame;
}/*}}}*/
